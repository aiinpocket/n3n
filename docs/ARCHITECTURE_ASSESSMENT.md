...
以下依「全功能開啟」（含網域模式 Caddy 與小應用沙盒）估算，基準為實測閒置用量：app 約 0.7GB、PostgreSQL/Redis/MongoDB 合計約 0.2GB。

### 記憶體估算公式

```
RAM 需求 ≈ 4GB（平台基礎，含 OS 與尖峰餘裕）
         + 0.5GB × 每個運行中的小應用容器
```

- 平台四容器的 compose 上限總和為 4.5GB（app 2G + pg 1G + mongo 1G + redis 0.5G），實際閒置只用約 1GB，上限是尖峰保護。
- 小應用每容器預設上限 512MB（`APPS_MEMORY_MB`）；注意一個 WordPress zip = wordpress + mariadb 兩個容器 = 1GB。
- Caddy 約 50MB，可忽略。
...
| 項目 | 大小 |
|------|------|
| OS + Docker + 平台映像 | 約 15GB |
| 資料庫（千級流程/執行紀錄） | 5–20GB |
| 作品庫（artifacts） | 依產出成長，影音為主時最吃空間，預留 30GB 起 |
| 小應用映像 | 每應用 0.3–1GB（nginx 50MB、wordpress 700MB、mariadb 350MB） |

### 建議規格（GCE 對照）

| 情境 | 規格 | 支撐能力 | 月費約 |
|------|------|----------|--------|
| 全功能小規模 | e2-standard-2（2 vCPU / 8GB）+ 100GB pd-balanced | 平台全功能 + 約 6–8 個小應用容器；10–50 活躍使用者 | $70–90 |
| 全功能標準（建議起點） | e2-standard-4（4 vCPU / 16GB）+ 200GB | 約 20 個小應用容器；數百活躍使用者 | $130–170 |
| 成長期 | e2-standard-8（8 vCPU / 32GB）+ 500GB | 約 50 個小應用容器 | $280–350 |

- CPU 很少是瓶頸：小應用每容器限 0.5 vCPU（`APPS_CPUS`），流程引擎跑 Virtual Threads，真正的等待都在外部 AI API。
- AI API 費用另計且與規格無關，永遠是最大宗支出。
- 小應用總量由 `APPS_MAX_PER_USER`（預設 2）與使用者數共同決定——這是唯一會線性吃掉記憶體的變數，開放註冊前先算：`使用者數 × 2 × 0.5GB` 是否在 RAM 預算內，必要時調低上限。
- 垂直升級（換機型）即可一路撐到 32 vCPU，不需要改架構；再往上才考慮前文的 Phase 2/3。
