...
-- plugin_installation: user plugin listing
CREATE INDEX IF NOT EXISTS idx_plugin_installation_user_id ON plugin_installation (user_id);

-- plugin_rating: rating queries
CREATE INDEX IF NOT EXISTS idx_plugin_rating_plugin_id ON plugin_rating (plugin_id);
...
-- execution_approval: pending approval queries
CREATE INDEX IF NOT EXISTS idx_execution_approval_status ON execution_approval (status);
CREATE INDEX IF NOT EXISTS idx_execution_approval_expires ON execution_approval (expires_at);

-- approval_action: approval action queries
CREATE INDEX IF NOT EXISTS idx_approval_action_approval_id ON approval_action (approval_id);

-- external_service: service listing
CREATE INDEX IF NOT EXISTS idx_external_service_created_by ON external_service (created_by, is_deleted);
CREATE INDEX IF NOT EXISTS idx_external_service_status ON external_service (status, is_deleted);
...
-- plugin_install_task: task listing
CREATE INDEX IF NOT EXISTS idx_plugin_install_tasks_user_id ON plugin_install_task (user_id);
CREATE INDEX IF NOT EXISTS idx_plugin_install_tasks_status ON plugin_install_task (status);
