...
@Component
@Slf4j
public class MySQLNodeHandler extends AbstractNodeHandler {
    ...
    public String getDescription() {
        return "Execute queries on MySQL databases.";
    }
    ...
    protected NodeExecutionResult doExecute(NodeExecutionContext context) {
        ...
        int port = getIntConfig(context, "port", 3306);
        ...
        try {
            ...
            return NodeExecutionResult.failure("MySQL driver not found. Please add mysql-connector-java dependency.");
        }
    ...
    private NodeExecutionResult executeQuery(Connection conn, String query, NodeExecutionContext context) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                log.debug("MySQL query returned {} rows", rows.size());
    ...
    private NodeExecutionResult executeUpdate(Connection conn, String query, NodeExecutionContext context) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            log.debug("MySQL update affected {} rows", affected);
            return NodeExecutionResult.success(output);
        }
    }
    ...
    private NodeExecutionResult executeInsert(Connection conn, String query, NodeExecutionContext context) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (!generatedKeys.isEmpty()) {
                output.put("insertId", generatedKeys.get(0));
            }

            log.debug("MySQL insert affected {} rows", affected);
            return NodeExecutionResult.success(output);
        }
    }
