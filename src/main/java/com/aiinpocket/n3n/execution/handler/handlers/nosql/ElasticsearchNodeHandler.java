...
public class ElasticsearchNodeHandler extends MultiOperationNodeHandler {
    ...
    /**
     * Build the client cache key. The readable prefix (host/port/username) is combined with a
     * SHA-256 digest of all authentication material (scheme, username, password, api-key). A
     * different password or api-key yields a different key, so a caller with the correct
     * host/username but a wrong secret can never reuse another user's authenticated client.
     */
    String generateCacheKey(Map<String, Object> credential) {
        ...
        String secretHash = CacheKeyUtil.sha256Hex(
            CacheKeyUtil.nullToEmpty(scheme) + "|"
                + CacheKeyUtil.nullToEmpty(username) + "|"
                + CacheKeyUtil.nullToEmpty(password) + "|"
                + CacheKeyUtil.nullToEmpty(apiKey));
        return String.format("%s:%s:%s:%s", host, port, username, secretHash);
    }
    ...
    void evictIdleClients() {
        ...
        clients.entrySet().removeIf(e -> {
            if (now - e.getValue().lastAccess > ttl) {
                try {
                    e.getValue().transport.close();
                } catch (Exception ex) {
                    log.warn("Error closing idle Elasticsearch client: {}", ex.getMessage());
                }
                return true;
            }
            return false;
    ...
    private static class ClientEntry {
        ...
        ClientEntry(ElasticsearchClient client, ElasticsearchTransport transport, long lastAccess) {
            this.client = client;
