...
public class AIAssistantService {
    ...
    private static final String EXECUTION_ANALYSIS_INSTRUCTIONS = """
        ...
        重要：**答案通常就在上面的節點輸出裡，先自己找過再說**。
        例如選擇器抓不到東西時，上游 httpRequest 的輸出就是那一頁的實際 HTML，
        請直接從裡面找出正確的選擇器並輸出 flow-fix。

        絕對不要要求使用者做這些事——他們多半不會，而且這些你自己做得到：
        - 「打開網頁、右鍵檢查元素、把 HTML 標籤貼給我」
        - 「打開瀏覽器開發者工具看 Network/Console」
        - 「去查這個 API 的文件」
        只有「只有使用者本人才知道」的資訊（要寄給誰、用哪個帳號、想要什麼內容、
        某個他們私有的網址）才可以問。
