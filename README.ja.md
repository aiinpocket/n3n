...
| `ADMIN_EMAILS` | `mopacke2422@gmail.com` | 管理者メールアドレスのリスト（カンマ区切り）。リスト内のアカウントはログイン時に自動的に管理者権限が付与されます |

### プラットフォーム共有 AI キーとメンバー管理

AI プロバイダーの API キーは管理者が「AI 設定」で一括設定し、全メンバーで共有します。メンバーが自分でキーを用意する必要はありません。「AI 使用量管理」ではプラットフォーム全体の使用量とメンバー別の内訳を確認できます（いずれも管理者専用）。管理者は「ユーザー管理」ページで他のメンバーに管理者ロールを付与できます。

### 共有リンクと共同編集

「マイフロー」の共有ダイアログから共有リンクを発行できます（閲覧／編集権限、有効期限の設定、いつでも取り消し可能）。リンクを渡した相手はログイン後、同じフローを一緒に編集できます。

### パーソナル AI メモリー

AI アシスタントはユーザーごとの好みや習慣を記憶します（ユーザー間で完全に分離）。使うほどフロー生成があなたに寄り添うようになります。「アカウント設定 → AI メモリー」でいつでも確認・編集・削除できます。
